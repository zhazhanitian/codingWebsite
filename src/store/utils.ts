...
// 提取函数参数
type GetParameters<T extends (...args: any) => any> = T extends (A: infer PA, B: infer PB) => any
  ? PB
  : never
...
interface Dispatch {
  <K extends keyof ActionsModules>(
    obj: { type: K; payload: ActionsModules[K] },
    options?: DispatchOptions
  ): Promise<any>
  <K extends keyof ActionsModules>(
    type: K,
    payload?: ActionsModules[K],
    options?: DispatchOptions
  ): Promise<any>
}
...
interface Commit {
  ...
  <K extends keyof MutationsModules>(
    type: K,
    payload?: MutationsModules[K],
    options?: CommitOptions
  ): unknown
}

interface ActionContextCommit<Module> {
  <K extends keyof Module>(
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    obj: { type: K; payload: GetParameters<Module[K]> },
    options?: CommitOptions
  ): unknown
  <K extends keyof Module>(
    type: K,
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    payload?: GetParameters<Module[K]>,
    options?: CommitOptions
  ): unknown
}
...
export { Getters, Actions, ActionContextCommit, Mutations, Commit, Dispatch, ActionContext }
